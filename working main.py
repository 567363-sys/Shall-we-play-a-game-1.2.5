...
def game_start():
  ...
  while True:
    ...
    if(goodblock3.xcor() < -150):
      goodblock3.setx(150)
    # FIX: goodblock4 was missing its boundary check
    ...
    if(down.xcor()>200):
      down.goto(-200,-170)
    # FIX: clear before writing HP each frame so text doesn't stack
    upHpTurtle.clear()
    upHpTurtle.write("Hp: "+str(upHp[0]), font=("Arial", 30, "normal"))
    downHpTurtle.clear()
    downHpTurtle.write("Hp: "+str(downHp[0]), font=("Arial", 30, "normal"))
  
    # FIX: iterate copies of the lists so we can safely remove mid-loop
    for b in topBullets[:]:
      b.sety(b.ycor()+10)
      if b.distance(up)<20:
        upHpTurtle.clear()
        ...
        b.ht()
        topBullets.remove(b)
      # FIX: remove bullets that go off screen
      elif b.ycor() > 320:
        b.ht()
        topBullets.remove(b)
...
bg.hideturtle()
bg.speed(0)
# FIX: only load gif if the file actually exists, otherwise the game crashes
if os.path.isfile("SECOND_background_space_invaders.gif"):
...
def draw_loading():
    ...
    screen.update()
    time.sleep(0.8)
    # FIX: schedule game_start via ontimer so it runs outside the click event
    # handler — calling it directly blocks turtle's event loop and crashes
    screen.ontimer(game_start, 0)
